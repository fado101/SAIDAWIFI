...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  // ✅ إضافة endpoint لمسح الـ cache من جانب الخادم
  app.post('/api/auth/clear-cache', async (req, res) => {
    console.log("🧹 Clear cache endpoint hit - clearing all stored tokens");
    res.json({ success: true, message: "Cache cleared successfully" });
  });

  // تسجيل الدخول الموحد لكل من الويب و Flutter  
  app.post('/api/login', async (req, res) => {
    // ✅ إضافة CORS headers مع logging إضافي
    const origin = req.get('origin');
    const userAgent = req.get('user-agent');
    console.log(`🌐 CORS Debug - Origin: ${origin}, User-Agent: ${userAgent}`);
    console.log(`🔧 Environment: ${process.env.NODE_ENV}, JWT Secret exists: ${!!process.env.JWT_SECRET}`);
    ...
    try {
      ...
      console.log("🔐 radiusApi.login result:", loginResult);

      if (!loginResult.success) {
        console.log(`❌ Login failed for ${username}:`, loginResult.message || "Unknown reason");
        return res.status(401).json({ 
          success: false, 
          message: loginResult.message || "البيانات المدخلة غير صحيحة" 
        });
      }

      // استخدام API يعمل بدلاً من getUserData الذي يفشل بـ HTTP 500
      const remainingData = await radiusApi.getRemaining(username);
      const userData = remainingData ? {
        username: remainingData.username || username,
        firstname: remainingData.username || username,
        lastname: '',
        email: remainingData.username || username,
        expiration: remainingData.expiration || '',
        srvid: remainingData.srvid || '1'
      } : {
        username: username,
        firstname: username,
        lastname: '',
  ...
  app.get('/api/auth/user', authenticateJWT, async (req: any, res) => {
    try {
      ...
      const fallbackUsername = req.user?.username || 'unknown';
      res.json({
  ...
  app.get('/api/user-profile', authenticateJWT, async (req: any, res) => {
    try {
      ...
      if (remainingData) {
        // استخراج البيانات من get_remaining_detailedok.php الذي يعمل
        res.json({
          success: true,
          data: {
            username: remainingData.username || username,
            firstName: remainingData.username || username, // استخدام username كـ firstName
            lastName: '',
            email: remainingData.username || username,
            expiration: remainingData.expiration || '',
            srvid: remainingData.srvid || '1',
            service: remainingData.service || ''
          }
        });
      } else {
        // fallback data
  ...
  // Server creation is now handled in index.ts
}
