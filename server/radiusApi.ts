...
export class RadiusApiService {
  async login(credentials: { username: string; password: string }): Promise<RadiusLoginResult> {
    try {
      ...
      // 🔒 SECURITY FIX: تشفير كلمة المرور بـ MD5 قبل الإرسال لتتطابق مع قاعدة البيانات
      const hashedPassword = CryptoJS.MD5(password).toString();
      console.log(`🔐 Password MD5 Hash: ${hashedPassword.substring(0, 8)}...`);
      ...
      formData.append('password', hashedPassword);  // ✅ إرسال كلمة المرور المشفرة

      const https = await import('https');
      const httpsAgent = new https.Agent({
        // ✅ إصلاح الاتصال مع API الخارجي كما طلب المستخدم
      ...
      // 📡 طباعة الرد الخام من login.php كما طلب المستخدم
      console.log("📡 login.php raw response:", loginText);
  ...
  async getUserData(username: string): Promise<RadiusUserData> {
    try {
      ...
      const https = await import('https');
      const httpsAgent = new https.Agent({
        // ✅ إصلاح مشكلة SSL certificate مثل login method
        rejectUnauthorized: false
      });
