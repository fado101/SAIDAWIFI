  ...
  async login(username: string, password: string): Promise<LoginResponse> {
    try {
      ...
      if (isDirectAPIEnvironment()) {
        // 🔐 PRODUCTION: Use secure directApiService for PHP API
        console.log('🌐 [SECURE] Using secure directApiService for PHP API');
        
        // Import directApiService here to avoid circular dependency
        const directApiServiceModule = await import('./directApiService');
        const response = await directApiServiceModule.default.login(username, password);
        
        if (response.success && response.data?.token && response.data?.user) {
          const { token, user } = response.data;
          
          // 🔐 SECURITY: Validate token from server
          if (!token || typeof token !== 'string' || token.length < 20) {
            console.error('🚨 [SECURITY ALERT] Invalid token received from PHP server');
            return {
              success: false,
          ...
          await this.saveAuth(token, userData);
          console.log('✅ [SECURE] Login successful via secure PHP API');
          console.log('👤 [SECURE] User data saved:', userData.username);
          ...
          console.error('❌ [SECURE] PHP API Login failed:', response.error);
          return {
            success: false,
            message: response.error || 'اسم المستخدم أو كلمة المرور غير صحيحة'
          };
        }
      } else {
        // 🔧 DEVELOPMENT: Use Node.js server
        console.log('🔧 [SECURE] Using Node.js server for development');
